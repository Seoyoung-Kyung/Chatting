...
public class ClientThread extends Thread {
	...
	public ChatRoomDisplay ct_chatRoom = null;
	public ChatRoomDisplay ct_chatRoom_2 = null;
	...
	private static MessageBox inviteBox;
	...
	private static final int REQ_INVITE = 1071; // 추가
	private static final int REQ_GIVEHOST = 1081; // 추가
	...
	private static final int YES_INVITE = 2071; // 추가
	private static final int NO_INVITE = 2072; // 추가
	private static final int YES_HOST = 2081;
	private static final int NO_HOST = 2082;
	...
	private static List<ClientThread> userList = new ArrayList<>();
	...
	public ClientThread(Socket var1) {
		try {
			this.ct_sock = var1;
			this.ct_in = new DataInputStream(this.ct_sock.getInputStream());
			this.ct_out = new DataOutputStream(this.ct_sock.getOutputStream());
	...
	public void run() {
		try {
			for (Thread var1 = Thread.currentThread(); 
				var1 == this.thisThread; Thread.sleep(200L)) {
				...
				switch (var4) {
					case 1061 :
						var21 = var3.nextToken(); // 초대 받은 사용자 ID 
						var22 = Integer.parseInt(var3.nextToken()); // 방 번호 
						var7 = var21 + "님으로 부터 파일전송을 수락하시겠습니까?";
					...
					case 1071 :
						...
						var7 = var21 + "님으로 부터 초대를 수락하시겠습니까 ?";
						int var32 = JOptionPane.showConfirmDialog(this.ct_waitRoom, var7, "초대하기", 0);
						if (var32 == 1) {
							try {
								this.ct_buffer.setLength(0);
								this.ct_buffer.append(2072);
								this.ct_buffer.append("|");
								this.ct_buffer.append(this.ct_logonID);
								this.ct_buffer.append("|");
								this.ct_buffer.append(var22);
							...
							try {
								this.ct_buffer.setLength(0);
								this.ct_buffer.append(2071);
								this.ct_buffer.append("|");
								this.ct_buffer.append(this.ct_logonID); // 초대 받을
								this.ct_buffer.append("|");
								this.ct_buffer.append(var22);
								this.ct_buffer.append("|");
								this.ct_buffer.append(var21); // 초대한 사람
								this.ct_buffer.append("|");
								this.ct_buffer.append(var11);
								this.send(this.ct_buffer.toString());
							} catch (IOException var13) {
...
//			             while (var29.hasMoreTokens()) {
//			            	String username = var29.nextToken();
//			            	if(this.ct_chatRoom.isAdmin) {
//			            		username += " ★"; // 방장 표시	
//							}
//			            	var26.addElement(username); 
//			             }
					...
					case 2072 :
						...
						// inviteBox.dispose();
						if (var5 == 3031) {
							var7 = var6 + "님이 초대를 거부하였습니다.";
							JOptionPane.showMessageDialog(this.ct_chatRoom, var7, "초대", 0);
							break;
						} else if (var5 == 3032) {
							var7 = var6 + "님은 대기자 목록에 존재하지 않습니다.";
							JOptionPane.showMessageDialog(this.ct_chatRoom, var7, "초대", 0);
							break;
						}
						break;
					case 2081 :
						var21 = var3.nextToken(); // 방장인 사용자의 ID 
						var22 = Integer.parseInt(var3.nextToken()); // 방 번호 
						var6 = var3.nextToken(); // 방장 권한을 위임받을 사용자의 ID 
						var99 = var3.nextToken();
//						String var100 = var3.nextToken(); 
//						clientThread = new ClientThread(var100);
//						ct_chatRoom_2 = new ChatRoomDisplay(clientThread);
//						ct_chatRoom_2.isAdmin = true;
//						if (this.ct_logonID == var21 && this.ct_chatRoom.isAdmin) {
//					        this.ct_chatRoom.isAdmin = false;
//					   
//					    } 
					
						if (this.ct_logonID.equals(var6)) {
							this.ct_chatRoom.isAdmin = true;
						} else {
							this.ct_chatRoom.isAdmin = false;
						}
						this.ct_chatRoom.messages.append("### 방장이 " + var21 + "님에서 " + var6 + "님으로 변경되었습니다. ###\n");
						this.ct_chatRoom.message.requestFocusInWindow();
					...
					case 2082 :
						var21 = var3.nextToken();
						var6 = var3.nextToken();
						JOptionPane.showMessageDialog(this.ct_chatRoom, "이미 방장인 사용자입니다. 다른 사용자를 선택해주세요.", "방장 권한 위임", JOptionPane.INFORMATION_MESSAGE);
						break;
				}
			}
	...
	public void requestGiveHost(String var1) {
		try {
			this.ct_buffer.setLength(0);
			this.ct_buffer.append(1081);
			this.ct_buffer.append("|");
			this.ct_buffer.append(this.ct_logonID);
			this.ct_buffer.append("|");
			this.ct_buffer.append(this.ct_roomNumber);
	...
	public void requestInvite(String var1) {
		inviteBox= new MessageBox(this.ct_chatRoom, "채팅방 초대", "상대방의 승인을 기다립니다.");
		inviteBox.show();

		try {
			this.ct_buffer.setLength(0);
			this.ct_buffer.append(1071);
			this.ct_buffer.append("|");
			this.ct_buffer.append(this.ct_logonID);
			this.ct_buffer.append("|");
			this.ct_buffer.append(this.ct_roomNumber);
