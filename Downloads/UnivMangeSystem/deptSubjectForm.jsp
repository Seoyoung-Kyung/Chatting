...
#input2{font-family: 'IBM Plex Sans KR', sans-serif;
  width: 70px;
  height: 29px;
  border-top: none;
  border-left: none;
...
<%
  ...
  String sql = "select sub_id , sub_name, sub_time, sub_info, credits, sub_state from subject where dept_id = ?";
...
<div class="topnav">    
    ...
    <h3 style="font-weight:bold;">&nbsp; 개설 과목 정보</h3><hr>
  </div>    
  <form action="searchSubject.jsp" method="post" style="float:left;">
  &nbsp;
        <select id="sk" name="sk">
            <option value="sub_id">과목코드</option>
            <option value="sub_name">과목명</option>
            <option value="credits">학점</option>
            <option value="sub_time">강의 시간</option>
            <option value="sub_info">강의실</option>
        </select>
	    <input id="input1" type="text" name="sv" autocomplete="off">
	    <input id="input2" type="submit" value="검색" >
      <a href ="subjectInsert.jsp"><i class="fas fa-plus-circle" style="font-size:28px; color:rgb(54, 54, 54); float:right;">&nbsp;&nbsp;&nbsp;</i></a>
    </form>
        <table class="type11">
          <thead>
               <tr>
         ...
         <%
            while (rs.next()) {
              String sub_state = rs.getString("sub_state");
              String sub_id = rs.getString("sub_id");
              String sub_name = rs.getString("sub_name");
              String credits = rs.getString("credits");
              String sub_time = rs.getString("sub_time");
              String sub_info = rs.getString("sub_info");
         ...
         <tr>
            ...
            <td><%=sub_state%></td>
            <td> 
              <a href ="subjectUpdate.jsp?sub_id=<%=sub_id%>&sub_name=<%=sub_name%>&credits=<%=credits%>&sub_time=<%=sub_time%>&sub_info=<%=sub_info%>"><i class="fas fa-edit" style="color:rgb(107, 107, 107);"></i></a>
              <a href ="subjectDelete.jsp?sub_id=<%=sub_id%>&sub_name=<%=sub_name%>&credits=<%=credits%>&sub_time=<%=sub_time%>&sub_info=<%=sub_info%>"><i class="fas fa-cog" style="color:rgb(107, 107, 107);"></i></a>
            </td>   
          </tr>
      </th>
