...
<body style="overflow-x: hidden">
  ...
  String sql = "select r.sub_id, s.sub_name, sum(r.middle_score), round(avg(r.middle_score),2), sum(r.final_score), round(avg(r.final_score),2), sum(r.attendance), round(avg(r.attendance),2), sum(r.other_score), round(avg(r.other_score),2), round(avg(r.average),2) from registration r inner join subject s on r.sub_id=s.sub_id where r.prof_id=? group by r.sub_id, s.sub_name";
...
<div class="card" style="height:394px; overflow: scroll;">
  <h3 class='blue'><b>&nbsp; 총점 / 평점 / 반별 성적</b></h3><hr width=113%>
  ...
  <table>
        ...
        <thead>
          <tr>
               ...
               <th scope="cols">중간총합</th>
               <th scope="cols">중간평균( /35)</th>
               <th scope="cols">기말총합</th>
               <th scope="cols">기말평균( /35)</th>  
               <th scope="cols">출석총합</th>
               <th scope="cols">출석평균( /20)</th> 
               <th scope="cols">기타총합</th>
               <th scope="cols">기타평균( /10)</th> 
               <th scope="cols">학점평균</th> 
               <th scope="cols">반별성적</th>    
         </tr>
        </div>
         <%
            while (rs.next()) {
         ...
         <tr>
            <td><%=sub_id%></td>
            <td><%=sub_name%></td>
            <td><%=rs.getString("sum(r.middle_score)")%></td>
            <td><%=rs.getString("round(avg(r.middle_score),2)")%></td>
            <td><%=rs.getString("sum(r.final_score)")%></td>
            <td><%=rs.getString("round(avg(r.final_score),2)")%></td>
            <td><%=rs.getString("sum(r.attendance)")%></td>
            <td><%=rs.getString("round(avg(r.attendance),2)")%></td>
            <td><%=rs.getString("sum(r.other_score)")%></td>
            <td><%=rs.getString("round(avg(r.other_score),2)")%></td>
            <td><%=rs.getString("round(avg(r.average),2)")%></td>
            <td><a href="seeclassTest.jsp?sub_id=<%=sub_id%>"><i class="fas fa-check-circle" style="color:lightblue"></i></a></td>
          </tr>
