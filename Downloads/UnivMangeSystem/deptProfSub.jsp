...
<div class="topnav">    
    <img src="logo.jpg">
      <ul id ="fromInsideOut" style="font-weight:bold;">
          ...
          <li class="active"><a href="deptProfSub.jsp">과목별 담당 교수 </a></li>
          <a href="Logout.jsp" class="signup">Logout</a>
        </ul>
</div>
  <div class="card" style="overflow:scroll; height:600px;">
  <div style="width:100%; float:center;">
      <h3 style="font-weight:bold;">&nbsp; 과목별 담당 교수 정보</h3><hr>
      </div>
  <form action="searchProfSub.jsp" method="post" style="float:left;">
  &nbsp;
        <select id="sk" name="sk">
            <option value="ps.sub_id">과목코드</option>
            <option value="s.sub_name">과목명</option>
            <option value="p.prof_name">담당 교수</option>
            <option value="ps.prof_id">교수번호</option>
        </select>
	    <input id="input1" type="text" name="sv" autocomplete="off">
	    <input id="input2" type="submit" value="검색">
   <a href ="addProfSub.jsp"><i class="fas fa-user-plus" style="float:right; font-size:28px; color:rgb(54, 54, 54);">&nbsp;&nbsp;&nbsp;</i></a>
    </form>
	<table class="type11">
			<tr>
...
<%
	...
	try {
		...
		String sql = "select ps.sub_id ,s.sub_name ,p.prof_name ,ps.prof_id from prof_subject ps inner join professor p on ps.prof_id=p.prof_id inner join subject s on ps.sub_id=s.sub_id  inner join department d on d.dept_id=s.dept_id where d.dept_id=? order by p.prof_id, s.sub_id";
	...
	%>
        <tr>
            ...
            <td><%=prof_id%></td>
            <td>
              <a href ="ProfSubUpdate.jsp?sub_id=<%=sub_id%>&sub_name=<%=sub_name%>&prof_id=<%=prof_id%>"><i class="fas fa-user-cog" style="color:rgb(107, 107, 107)"></i></a>
            </td>
