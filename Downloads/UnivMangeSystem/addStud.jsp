...
#s_id, #s_name, #s_jumin, #s_address, #s_tel, #s_email, #s_grade, #s_class {
  font-family: 'IBM Plex Sans KR', sans-serif;
  width: 180px;
  height: 29px;
  border-top:none;
...
<div class="topnav">    
 ...
 <div style="width:100%; float:center;">
    <h3 style="font-weight:bold;">&nbsp; 학생 추가
    <a href ="studMgt.jsp"><i class="fas fa-arrow-left" style="font-size:28px; color:rgb(54, 54, 54); float:right;">&nbsp;&nbsp;&nbsp;</i></a>
    </h3><hr>
  </div>
	<form method="post" action="addStudPro.jsp">
  <table class="type11">
  &nbsp;
      <tr>
          <th scope="cols">학번</th>
          <td>        
            <input type="text" name ="s_id" id="s_id" placeholder="학번" autocomplete="off">
          </td>
      </tr>
      <tr>
          <th scope="cols">학생명</th>
          <td>    
            <input type="text" name ="s_name" id="s_name" placeholder="학생명" autocomplete="off">
          </td>
      </tr>
      <tr>
          <th scope="cols">담당 교수 번호</th>
          <td>
...
<%
	try {
		...
		String sql = "select distinct prof_id, prof_name from professor where dept_id= ?";
...
%>
        <option value="<%=prof_id%>"><%=prof_id%> / <%=prof_name%></option>
    <%  } 
}catch(Exception e){
	e.printStackTrace();
}finally{
...
%>
      ...
      <tr>
          ...
          <td>
            <input type="text" name ="s_jumin" id="s_jumin" placeholder="주민번호" autocomplete="off" >
          </td>
      </tr>
      <tr>
          <th scope="cols">주소</th>
          <td>
            <input type="text" name="s_address" id="s_address" placeholder="주소" size="70" style="position:relative; left:42px;" autocomplete="off">&nbsp;
      ...
      <tr>
          ...
          <td>
            <input type="text" name ="s_tel" id="s_tel" placeholder="전화번호" autocomplete="off" >
          </td>
      </tr>
      <tr>
          <th scope="cols">이메일</th>
          <td>
            <input type="text" name ="s_email" id="s_email" placeholder="이메일" autocomplete="off" >
          </td>
      </tr>
      <tr>
          <th scope="cols">학년</th>
          <td>
            <input type="text" name ="s_grade" id="s_grade" placeholder="학년" autocomplete="off" >
          </td>
      </tr>
      <tr>
          <th scope="cols">반</th>
          <td>
            <input type="text" name ="s_class" id="s_class" placeholder="반" autocomplete="off" >
          </td>
      </tr>
      </table>
  ...
  function kakaopost() {
      new daum.Postcode({
          oncomplete: function(data) {
             document.querySelector("#s_address").value =  data.address
          }
      }).open();
  }
  </script>
